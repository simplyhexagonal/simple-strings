...
export const diacriticReplacePatterns: MultiReplaceSyncPatterns = [
  [
 ...
 */
export const removeDiacritics = (string: string, limitLength: boolean = true) => {
  if (limitLength && string.length > 2048) {
    // Limits are good, we don't want to allow script kiddies to DDOS
  ...
  return multiReplaceSync(string, diacriticReplacePatterns);
};
...
export const makeUrlSafe = (string: string) => {
  ...
  // Replace spaces and special characters with "-" character, this allows search
  // engines to more easily read words present in string.
